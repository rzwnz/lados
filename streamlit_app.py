...
with col1:
    ...
    if uploaded_file is not None:
        ...
        if file_size_mb > MAX_UPLOAD_SIZE_MB:
            st.error(f"File too large ({file_size_mb:.2f} MB). Maximum size: {MAX_UPLOAD_SIZE_MB} MB")
        else:
                            ...
                            with st.expander(f"Image {i+1}: {result.get('filename', 'unknown')}"):
                                if "error" in result:
                                    ...
                                    if "top_class" in result:
                                        ...
                                        st.metric("Confidence", f"{result.get('top_score', 0) * 100:.2f}%")
                                    
                                    if "predictions" in result:
                                        pred_df = pd.DataFrame(result["predictions"][:5])
                                        pred_df["score"] = pred_df["score"].apply(lambda x: f"{x * 100:.2f}%")
